...
const Home = () => {
    const { loading, error, ingresosPorMoneda, clientesActivosCount, totalGastos } = useDashboardKpis()
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card className="p-4">
                    ...
                    {!gastosCat.loading && gastosCat.dataset.length > 0 && (
                        ...
                        <Card className="p-4">
                            <div className="text-sm font-semibold mb-2">Clientes activos</div>
                            <div className="text-4xl font-black text-gray-900">{loading ? '—' : clientesActivosCount}</div>
                            <div className="text-xs text-gray-500 mt-1">Clientes con flag activo=true</div>
                        </Card>
            </div>
        </div>
    )
