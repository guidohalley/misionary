...
test.describe('Admin - ABM Proveedor', () => {
  ...
  test('Admin: Crear - Editar - Eliminar Proveedor (sin basura)', async ({ page }) => {
    ...
    // Seleccionar áreas de especialidad (multi-select de React-Select)
    ...
    if (await areasContainer.count() > 0) {
      console.log('  Agregando areas: Frontend, Backend, Diseñador Grafico');
      ...
      // Seleccionar Diseñador Gráfico
      await areasContainer.click();
      await page.waitForTimeout(300);
      await page.locator('div[class*="option"]:has-text("Diseñador Gráfico")').click();
      await page.waitForTimeout(300);
    ...
    // Hacer click en el botón de editar (segundo botón de la fila)
    const editButton = proveedorRow.locator('button').nth(1);
    ...
    // Editar áreas - quitar Backend y agregar Paid Media
    const areasEditContainer = page.locator('label:has-text("Áreas")').locator('..').locator('[class*="control"]').first();
    
    if (await areasEditContainer.count() > 0) {
      console.log('  Editando areas: quitando Backend, agregando Paid Media');
      
      // Quitar Backend - buscar el tag "Backend" y su botón X
      const backendTag = page.locator('[class*="multiValue"]:has-text("Backend")');
      if (await backendTag.count() > 0) {
        const removeBtn = backendTag.locator('[class*="remove"], svg').first();
        await removeBtn.click();
        await page.waitForTimeout(300);
        console.log('    Backend eliminado');
      }
      
      // Agregar Paid Media
      await areasEditContainer.click();
      await page.waitForTimeout(300);
      await page.locator('div[class*="option"]:has-text("Paid Media")').click();
      await page.waitForTimeout(300);
      console.log('    Paid Media agregado');
    ...
    console.log('OK - Proveedor editado (con nuevas areas)');
    ...
    // Hacer click en el botón de eliminar (tercer botón de la fila)
    const deleteButton = proveedorRow2.locator('button').nth(2);
